...
- Desktop UI should use `dev-docs/go-docs/specs/daemon-api.md` as the current integration contract.

## Future Go Layout Cleanup

The current Go code uses a standard Go layout:
...
This is valid and should not be reorganized inside documentation or release-readiness changes. As the Go product core has grown, `internal/` has become flat enough that a separate cleanup branch would make future navigation easier.
...
- Do it on a dedicated branch, for example `codex/go-layout-cleanup`.
- Keep public CLI behavior, daemon API, JSON schemas, provider metadata, worker contracts, and exit codes unchanged.
- Prefer moving low-coupling packages first: `paths`, `downloads`, `procutil`, `ffmpeg`, and runtime adapters.
- Move high-coupling packages such as `jobs`, `providers`, and `models` only after the first slice passes.
- Update imports mechanically and run `go test ./...` after each slice.
- Keep documentation path updates in the same cleanup branch, but do not mix behavior changes into the move.
