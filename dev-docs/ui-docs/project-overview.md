...
Electron 实现前固定按以下顺序阅读 `dev-docs/ui-docs` 顶层文档：
...
`dev-docs/ui-docs/project-overview.md` 是 Electron 规划和实现的入口文档。

## 后续实现轮次

Round 8 到 Round 10.5 已经完成，`dev-docs/ui-docs` 后续只规划 Electron 相关实现。后续收敛为三轮：
...
1. 将 `dev-docs/ui-docs/prototype` 中的桌面原型落地为 Electron 应用。
...
- 基于 `dev-docs/ui-docs/prototype` 的 Electron 桌面应用。
- 首次启动 / 环境检查流程。
- 主界面 / 一键生成字幕流程。
- 详细设置折叠面板。
...
1. Electron 应用可以跑通 `dev-docs/ui-docs/prototype` 覆盖的主要页面和状态。
