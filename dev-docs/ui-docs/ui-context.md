...
本文档记录 Fast Sub Electron 应用的基础 UI token。当前 token 参考 `dev-docs/ui-docs/prototype/index.html` 和 `dev-docs/ui-docs/prototype/FastSub UI v2.html` 中的原型设计。生产实现可以整理命名，但视觉语义应保持一致。
