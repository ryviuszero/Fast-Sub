...
分支内每完成一个主要 workstream，都必须更新 `dev-docs/ui-docs/project-tracker.md`，记录完成内容、验证命令、剩余问题和下一步。
...
| 单元 | 名称 | 范围 | 验收 |
|---|---|---|---|
| 12.1 | Daemon API contract 扩展 | 更新 `dev-docs/go-docs/specs/daemon-api.md`，补 `model_install`、`translate_srt`、`burn_in`、config、transient secret reference 边界和 fixtures | UI 不再猜 API；fake daemon fixtures 覆盖新增 job type |
...
- `dev-docs/go-docs/specs/daemon-api.md` 写明 `model_install`、`translate_srt`、`burn_in` 的 request/result/event shape。
...
- 更新 `dev-docs/ui-docs/project-tracker.md`：记录 Round 12 spec、已定决策、待实现 workstreams。
- 更新 `dev-docs/ui-docs/architecture.md`：把 Round 12 后真实 daemon client、model_install job、secret storage、config adapter 写入架构。
- 更新 `dev-docs/ui-docs/code-standards.md`：明确 secret storage、daemon IPC、SSE client、config adapter 的代码规则。
- 更新 `dev-docs/go-docs/specs/daemon-api.md`：新增 `model_install`、`translate_srt`、`burn_in`、model install endpoint/job contract 和 config/secret 边界。
