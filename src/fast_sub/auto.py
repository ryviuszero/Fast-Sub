...
from fast_sub.errors import SubGenError, WorkerRunnerError
from fast_sub.media import ensure_media_tools, is_media_file, probe_media
from fast_sub.model_manager import ModelManagerError, install_model
        ...
        if resolution.provider_location == "api":
            steps.append(
                AutoStep(
                    ...
                    action_hint=(
                        "Choose --provider local-faster-whisper. "
                        "--yes never authorizes API upload."
                    ),
                )
            )
            if options.dry_run:
