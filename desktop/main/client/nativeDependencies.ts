...
export function prependNativeDependencyPath(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const binDirs = [ffmpegBinDirectory(), whisperCPPBinDirectory()];
...
export async function ensureFFmpegInstalled(): Promise<NativeDependencyStatus> {
  ...
  if (process.platform !== "win32") {
    return {
      ...
      installing: false,
      progressPercent: 0,
      message: "Automatic FFmpeg installation is currently implemented for Windows only.",
...
export async function installFFmpegWithPackageManager(manager: FFmpegPackageManager): Promise<NativeDependencyStatus> {
  if (process.platform !== "win32") {
    return { available: false, installedNow: false, installing: false, progressPercent: 0, message: "Package manager FFmpeg install is currently implemented for Windows only.", logs: installState.logs };
  }
...
export async function ensureWhisperCPPInstalled(): Promise<NativeDependencyStatus> {
  ...
  if (process.platform !== "win32") {
    return {
      ...
      installing: false,
      progressPercent: 0,
      message: "Automatic whisper.cpp installation is currently implemented for Windows only.",
      logs: whisperCPPInstallState.logs
    };
  }
...
async function checkFFmpegAvailable(): Promise<NativeDependencyStatus> {
  ...
  return {
    ...
    installedNow: false,
    installing: false,
    progressPercent: ffmpeg && ffprobe ? 100 : 0,
    binDir,
    message: ffmpeg && ffprobe ? "FFmpeg is available." : "FFmpeg or ffprobe is missing.",
    logs: installState.logs
  };
}

async function checkWhisperCPPAvailable(): Promise<NativeDependencyStatus> {
  const binDir = whisperCPPBinDirectory();
  const env = prependNativeDependencyPath({ ...process.env });
...
async function installWindowsFFmpeg(): Promise<void> {
  ...
  if (aria2Path) {
    await downloadFileWithAria2(windowsFFmpegURL, zipPath, aria2Path);
  } else {
...
function startInstallTask(): void {
  ...
  installTask = installWindowsFFmpeg()
    .then(() => {
...
function startWhisperCPPInstallTask(): void {
  ...
  whisperCPPInstallTask = installWindowsWhisperCPP()
    .then(() => {
...
async function ensureAria2Installed(): Promise<string | null> {
  ...
  if (await canRun("aria2c", process.env)) {
...
async function downloadFileWithPowerShell(url: string, target: string, label: string, progressStart: number, progressEnd: number, logProgress = pushLog): Promise<void> {
  ...
  if (parsed.protocol !== "https:" || !["www.gyan.dev", "github.com"].includes(parsed.hostname)) {
...
async function downloadFileWithAria2(url: string, target: string, aria2Path: string): Promise<void> {
  const parsed = new URL(url);
  if (parsed.protocol !== "https:" || parsed.hostname !== "www.gyan.dev") {
    throw new Error("Refusing to download FFmpeg from an unapproved source.");
  }
  ...
  if (total > 0 && info.size !== total) {
    throw new Error(`FFmpeg aria2 download was incomplete (${info.size}/${total} bytes).`);
  }
}
...
function progressLogPrefix(message: string): string {
  ...
  if (message.startsWith("aria2 正在下载 FFmpeg：")) {
    return "aria2:ffmpeg";
  }
...
async function findWhisperCPPExecutable(root: string): Promise<string | null> {
  for (const name of ["whisper-cli.exe", "main.exe", "whisper-cpp.exe"]) {
...
async function findWhisperCPPBin(root: string): Promise<string | null> {
  const entries = await readdir(root, { withFileTypes: true });
  const hasWhisperCLI = entries.some((entry) => entry.isFile() && ["whisper-cli.exe", "main.exe", "whisper-cpp.exe"].includes(entry.name.toLowerCase()));
  if (hasWhisperCLI) {
    return root;
  }
  for (const entry of entries) {
