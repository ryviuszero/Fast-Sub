...
```text
47 errors in 10 files
```

The largest remaining groups are benchmark numeric statistics, `object` to `float` or
`int` conversions from parsed metadata, and a few old benchmark metrics re-export names.
