...
| R13-AUTO-002 | `PASS` | `cd desktop && npm run typecheck` | 通过 | 2026-05-21：TypeScript strict check 在 macOS 原生依赖安装修复后通过。 |
...
| R13-AUTO-018 | `PASS` | macOS arm64 release script smoke | 通过 | 2026-05-21：macOS arm64 release machine 上完成 `prepare:python-runtime`、`package:dir`、`smoke:packaged`、`smoke:translation-model-failure` 和 `package`。修复 macOS Python runtime 外部 symlink 后，`codesign --deep --strict` 不再报 `invalid destination for symbolic link in bundle`。 |
...
| R13-AUTO-012 | `PASS` | Packaged native dependency smoke harness | 通过 | 2026-05-21：`npm run smoke:native-deps` 已支持平台自适应 packaged app。Windows 覆盖 FFmpeg/aria2 fallback/whisper.cpp fallback 首装；macOS 覆盖 FFmpeg/FFprobe 从 GitHub gz binary 下载到 app 私有 userData，并验证 app bundle 内置 whisper.cpp runtime。该命令访问真实下载源，只作为手动 release smoke，不进默认 CI。验证 `npm run typecheck`、`npm run package:dir`、`npm run smoke:native-deps`、`npm run smoke:packaged`。 |
...
| R13-PKG-007 | `PASS` | FFmpeg/FFprobe 首装 | 隔离 userData 后运行 `npm run smoke:native-deps` 的 `ffmpeg` 场景 | 可安装或给出明确修复动作 | 通过 | Windows 2026-05-19：先下载 app 私有 aria2，再用 aria2 下载 gyan.dev FFmpeg essentials；`ffmpeg.exe`/`ffprobe.exe` 发布到 app 私有 `native-binaries/ffmpeg/bin`。macOS 2026-05-21：强制忽略系统 FFmpeg 后，从 `github.com/eugeneware/ffmpeg-static` 下载 `ffmpeg-darwin-arm64.gz` 和 `ffprobe-darwin-arm64.gz`，发布到隔离 userData 的 app 私有 `native-binaries/ffmpeg/bin`；`available=true`，`installedNow=true`。 |
...
| R13-PKG-009 | `PASS` | whisper.cpp binary | 隔离 userData 后运行 `npm run smoke:native-deps -- whisper-cpp` 或默认 native deps smoke | 可用且不要求用户安装 Homebrew/Scoop/编译工具；模型仍按需下载 | 通过 | Windows 2026-05-19：download-only fallback 从 ggml-org/whisper.cpp GitHub release 下载 Windows x64 binary zip，解压 `whisper-cli.exe` 到 app 私有 userData。macOS 2026-05-21：`prepare:whisper-cpp-runtime` 从 pinned `v1.8.4` source 构建 `whisper-cli` + dylib，打包到 `Contents/Resources/bin/darwin-arm64/whisper-cpp`；最终 native smoke 返回 `available=true`、`installedNow=false`、`binDir=.../Contents/Resources/bin/darwin-arm64/whisper-cpp`，无需 Homebrew。 |
...
| R13-PKG-011 | `PASS` | macOS arm64 dmg artifact | 在 macOS arm64 执行 `npm run package:dir && npm run smoke:packaged && npm run package` | app 启动、daemon ready、签名/公证状态有记录 | 通过 | 2026-05-21：生成 `desktop/dist-release/FastSub-Desktop-0.13.0-macos-arm64.dmg`（约 625 MB）和 `.blockmap`。`npm run smoke:packaged` 验证 packaged app、daemon ready/repair、Python runtime、SSE disconnect 和 `events_lost`；`npm run smoke:translation-model-failure` 验证缺 NLLB 模型返回 `missing_model`。 |
| R13-PKG-012 | `PASS` | macOS executable permissions | 在 macOS arm64 app bundle 内检查 daemon/Python/native binaries | 可执行权限正确，未被 quarantine 阻断，resources 定位正确 | 通过，Gatekeeper 未通过 | 2026-05-21：`test -x` 验证 `Contents/Resources/bin/darwin-arm64/fast-sub-go` 和 `Contents/Resources/python/darwin-arm64/bin/python` 可执行；`codesign --verify --deep --strict --verbose=2` 通过；`spctl --assess --type execute --verbose` 返回 `internal error in Code Signing subsystem`，当前仍按 ad-hoc signed / not notarized internal smoke 处理。 |
| R13-PKG-013 | `RETEST` | macOS process cleanup | macOS arm64 上运行任务后取消/退出/repair | 不遗留 daemon/Python/ffmpeg/whisper.cpp 子进程 | 基础 repair smoke 通过，真实长任务待测 | 2026-05-21：`npm run smoke:packaged` 覆盖 packaged app repair 和 daemon 重启，但没有运行真实 ASR/GPU/FFmpeg/whisper.cpp 长任务；真实取消/退出清理仍需后续手动 smoke。 |
| R13-PKG-016 | `PASS` | macOS DMG mount check | `hdiutil attach` 生成的 dmg | dmg 可挂载，包含 `Fast Sub.app` 和 Applications link | 通过 | 2026-05-21：dmg read-only 挂载到 `/Volumes/Fast Sub 0.13.0-arm64`，包含 `Fast Sub.app` 和 `Applications -> /Applications`，随后 `hdiutil detach` 成功。 |
| R13-PKG-017 | `PASS` | macOS package-manager actions | macOS 环境检查页缺 FFmpeg 时查看修复动作 | 不显示 Windows 专用 Scoop/Winget/Chocolatey；提供 Homebrew 手动入口，自动修复走 app 私有下载 | 通过 | 2026-05-21：`FFmpegPackageManager` contract 增加 `brew`，main process 只允许 macOS 使用 `brew install ffmpeg`；renderer 按 `environment.os` 展示 Homebrew 或 Windows 三种包管理器。自动修复优先 app 私有目录/系统 Homebrew 路径，缺失时下载 macOS FFmpeg/FFprobe gz binary。 |
